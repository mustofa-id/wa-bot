...
- **Notes** (`plugins/notes.ts`): `!notes add|ls|<id>|update|rm`. Per-chat shared notes with title + content. Sequential integer IDs. SQLite-backed (`notes.db`). Interactive add (title + content prompts) and update (show current, prompt new). `notes.spec.ts` tests via exported CRUD helpers.
