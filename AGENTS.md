...
- **SQLite**: `lib/utils.ts:useSqlite(name)` returns `Promise<DatabaseSync>` with `journal_mode=WAL` and `synchronous=NORMAL`. Called at module top-level (with `await`) in `lib/auth.ts`, `lib/users.ts`, `plugins/reminder.ts`, `plugins/prayers.ts`, `plugins/notes.ts`.
...
- **Reminders**: `plugins/reminder.ts` — `!reminder <waktu> [tanggal]`, `!reminder ls`, `!reminder rm <id>`. Text from quoted message or prompt. Time parsing via regex (24h `HH:mm`/`HHmm`, date `YYYY-MM-DD`/`DD-MM-YYYY`/etc.) with date aliases (`besok`/`tomorrow` = +1 day, `lusa`/`dayafter` = +2 day, case-insensitive). SQLite-backed, survives restarts. `remind_at` stored as UTC (`YYYY-MM-DD HH:mm:ss`); `fmtDateString` treats string inputs as UTC and formats in configured TZ.
...
- **Notes** (`plugins/notes.ts`): `!notes add|search|<id>|update|rm`. Per-chat shared notes with title + content. Sequential integer IDs. SQLite-backed (`notes.db`). Interactive add (title + content prompts) and update (show current, prompt new). `!notes` shows list; `!notes search <query>` searches title and content (case-insensitive LIKE). `notes.spec.ts` tests via exported CRUD helpers.
- **Auto-reconnect**: On connection close, bot waits 5s and restarts unless statusCode 401 (logout). **Concurrent reconnect guard**: `starting` flag prevents duplicate `startBot()` instances. `connection.update` handler wrapped in try/catch to prevent crash from unhandled rejection.
